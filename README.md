...
**A local-first RAG system** that indexes Obsidian notes into ChromaDB, exposes them to Claude Code via MCP, and auto-syncs Claude/Codex session summaries back into Obsidian — forming a closed loop of engineering knowledge.
...
│                         OBSIDIAN VAULT                                    │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌────────────────┐  │
│  │  Projects/   │  │  Knowledge/ │  │    Logs/    │  │  System/ skip  │  │
...
Indexes the configured Obsidian folders (`Projects/`, `Knowledge/`, `Prompts/`, and `Logs/` by default) into a local ChromaDB vector store.
...
Reads Claude Code and Codex session logs from the last 7 days (`~/.claude/projects/**/*.jsonl`, `~/.codex/sessions/**/*.jsonl`), filters through a 3-stage pipeline, and writes aggregated daily notes back to Obsidian.

#### Stage 1 — Heuristic Filter (Structural)

| Check | Trigger | Result |
| --- | --- | --- |
| No tool calls | Zero Claude `tool_use` blocks or Codex function calls | **TRASH** |
| Loop detection | Same shell command repeated >10x consecutively | **TRASH** |
| Git delta | `git diff --name-only` on session `cwd` is empty | **RESEARCH** |

#### Stage 2 — Outcome Signal

- **Exit codes:** Non-zero Claude hook exit code or Codex command exit code → flagged as failure
...
| **RESEARCH** | `Knowledge/Research Sessions - YYYY-MM-DD.md` | ✅ Indexed via `Knowledge/` | Permanent |
| **PROMPT PATTERNS** | `Prompts/Prompt Patterns - YYYY-MM-DD.md` | ✅ Indexed via `Prompts/` | Permanent |
| **PROJECT NOTES** | `Projects/<repo>/Session Notes - YYYY-MM-DD.md` | ✅ Indexed via `Projects/` | Permanent |
| **TRASH** | Provider archive (`~/.claude/archive/` or `~/.codex/archived_sessions/`) | ❌ Not indexed | Archived |

#### Output Format

Verified KNOWLEDGE sessions are written as one Obsidian note per calendar day, aggregating all sessions across all repos:
...
RESEARCH sessions are written to `Knowledge/Research Sessions - YYYY-MM-DD.md` with `outcome: "research"` and `session_count` metadata instead of raw session ids. The visible body does not include a session inventory.

Prompt-worthy user turns are detected with heuristics for clear questions, feedback/corrections, file paths, tickets, errors, payloads, and other concrete context. Those are distilled into `Prompts/Prompt Patterns - YYYY-MM-DD.md`.

When a session's repo matches an existing `Projects/<repo>/` folder, the pipeline also writes a project-scoped note to `Projects/<repo>/Session Notes - YYYY-MM-DD.md`.
...
After summarization, the pipeline appends a `## Related` section with `[[wikilinks]]` to matching existing notes from configured vault folders.
...
| Time | Script | Purpose |
| --- | --- | --- |
| Saturday 23:30 | `sync_sessions.py` | Scan last 7 days of Claude/Codex sessions, filter, classify, write daily notes |
...
├── sync_sessions.py                             # Claude/Codex session → Obsidian note pipeline
...
├── Prompts/         # Reusable prompting patterns
│   └── Prompt Patterns - YYYY-MM-DD.md
├── Logs/            # Auto-generated daily session notes (from sync_sessions.py)
└── System/
    ├── Pipeline Plan.md
    └── Staging/     # reserved for temporary notes
```
...
- 🧹 **Research pruning** — Optional cleanup for stale research notes if `Knowledge/` gets noisy

---

## What NOT to do

- Do not run `ingest.py` against `System/` — operational metadata, not durable knowledge.
